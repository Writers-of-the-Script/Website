...
function configItemToEntry(
    ...
    if ("link" in item) {
        ...
        return groupFromAutogenerateConfig(
            item,
            locale,
            routes,
            currentPathname,
    ...
    if (!route) {
        ...
        if (hasExternalSlashes) {
            throw new AstroError(
                ...
                `Please try updating \`"${item.slug}"\` to \`"${stripLeadingAndTrailingSlashes(
                    item.slug,
                )}"\`.`,
            );
        } else {
...
function getRoutePathRelativeToCollectionRoot(
    ...
    return project.legacyCollections
        ...
        : // For collections with a loader, use a localized filePath relative to the collection
          localizedId(
              route.entry.filePath.replace(
                  `${docsCollectionPathFromRoot}/`,
                  "",
              ),
              locale,
          );
...
function treeify(
    ...
    routes
        ...
        .map(
            (doc) =>
                [
                    getRoutePathRelativeToCollectionRoot(doc, locale),
                    doc,
                ] as const,
        )
...
function setIntermediateSidebarCurrentEntry(
    ...
    for (const entry of intermediateSidebar) {
        if (
            entry.type === "link" &&
            pathsMatch(encodeURI(entry.href), pathname)
        ) {
