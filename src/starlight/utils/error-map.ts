...
const errorMap: z.ZodErrorMap = (baseError, ctx) => {
    ...
    if (baseError.code === "invalid_union") {
        ...
        const messages: string[] = [
            prefix(baseErrorPath, "Did not match union."),
        ];
        ...
        if (details.length === 0) {
            ...
            if (expectedShapes.length) {
                details.push(
                    "> Expected type `" + expectedShapes.join(" | ") + "`",
                );
...
const getTypeOrLiteralMsg = (error: TypeOrLiteralErrByPathEntry): string => {
    ...
    if (
        typeof error.received === "undefined" ||
    ...
    switch (error.code) {
        case "invalid_type":
            return `Expected type \`${unionExpectedVals(
                expectedDeduped,
            )}\`, received \`${stringify(error.received)}\``;
