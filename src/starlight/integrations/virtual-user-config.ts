...
export function vitePluginStarlightUserConfig(
     ...
     * // => '"/users/houston/docs/node_modules/nulledlight/utils/git.ts"'
     */
    ...
    const modules = {
        ...
        "virtual:starlight/collection-config": `let userCollections;
			try {
				userCollections = (await import(${JSON.stringify(
            ...
            opts.routeMiddleware
                .reduce(
                    ([imports, entries], id, index) => {
                        ...
                        imports += `import { onRequest as ${importName} } from ${resolveId(
                            id,
                        )};\n`;
                        entries += `\t${importName},\n`;
        ...
        "virtual:starlight/pagefind-config": `export const pagefindUserConfig = ${JSON.stringify(
            opts.pagefind || {},
        )}`,
